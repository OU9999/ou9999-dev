...
- 기본 텍스트: 중성 소프트 화이트
- UI 구조색: 중성 화이트와 무채색 회색
- Blue Steel: 에디토리얼 이미지와 등록된 그라데이션 전용
...
| 가장 깊은 그림자 | `mineral-soot` | `#030303` |
| 프레임·상승 표면 | `mineral-graphite` | `#111111` |
| 기본 텍스트 | `google-paper`, `mineral-bone` | `#E8E8E8` |
| 보조 텍스트 | `google-muted`, `mineral-muted` | `#909090` |
| Blue Steel 그라데이션 스톱 | `bg-mineral-lettering` 내부 | `#A2ADB3` |

- Blue Steel 색상값은 단색 UI 토큰으로 노출하지 않고 이미지와 등록된 그라데이션 내부에서만 사용
- 배경 워시의 모든 구조색은 RGB 채널 값이 같은 무채색만 사용
- UI 텍스트·아이콘·구조선: `google-paper`와 opacity 조합
- 링크·버튼·focus 강조: 화이트 명도 상승 또는 등록된 텍스트 그라데이션
...
- 운영 UI: 중성 화이트·미색·흑연색
- 운영 UI 위계: 색상 변경보다 opacity·명도·질감 우선
- 단색 Blue Steel UI: 텍스트·아이콘·테두리·구분선·배경·그림자 적용 금지
- Blue Steel 허용: 대표 이미지·MDX 이미지·등록된 텍스트 그라데이션
- 순백 사용: 선택·hover·focus와 제한된 최고 명도 강조
- 썸네일 팔레트와 UI 팔레트의 분리
...
- 토큰: `bg-mineral-wash`
- 방향: `112deg`
- 색상: `#0A0A0A 0%` → `#111111 48%` → `#080808 100%`
...
- 섹션마다 끊기는 별도 배경
- 썸네일보다 강한 배경 색상
- 청록·인디고·Blue Steel 색조가 식별되는 구조적 배경
- 누런 종이·갈색 안료 중심의 표면
...
| 장식 붓질 | 브랜드 리듬 | 단단한 초크 질감, 마른 가장자리, 중성 화이트·미색 |
...
- 흑연색 표면 위의 얇은 붓 프레임
- 헤더 프레임·브랜드 심벌·내비게이션·활성 밑줄: 중성 화이트
- 헤더 명도 위계: 색상 변경보다 투명도와 붓 질감 우선
- 헤더 hover·focus: Blue Steel 전환 없이 화이트 명도 상승
- 푸터 붓질·아이콘·구분선: 헤더와 동일한 중성 색온도
- 본문보다 높은 z축
- 과도하지 않은 흐림과 투명도
- 브랜드 심벌·영문 링크 중심
- 푸터 상단의 넓고 낮은 붓질 경계
...
- 미색 기본 텍스트
- Blue Steel 텍스트 그라데이션 링크
- 중성 흑연색 코드블록·인라인 코드 표면
