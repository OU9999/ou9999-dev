...
또한, 클라이언트 컴포넌트를 리프노드에 가깝게 설계해야하는건 성능 향상은 물론, App Router 가 아니더라도 지향해야하는 설계 방향성이라고 생각합니다.
...
이 구조는 App Router가 지향하는 컴포넌트 패턴과 유사하다고 생각합니다.
...
Parallel Routes가 저에겐 큰 장점이었던것 같습니다. 새로운 블로그는 심플을 추구하였는데,\
...
이제는 직접 문제를 탐구하며, 다양한 동작 원리를 쉽게 풀어서 설명하는 글을 작성하고 싶은 욕심이 있습니다.
