---
title: CLOVA OCR 최소 비용과 제한된 기능으로 구현하는 신분증 OCR 알고리즘
description: 제한된 기능으로 원하는 데이터를 추출하는 과정과 후기
date: "2024-06-30"
tags:
  - clova-ocr
  - algorithm
...
이 글은 CLOVA OCR 기본 사용법이나 프론트엔드 구현 코드에 관한 글이 아닌, 적은 요금으로 신분증의 이름과 주민등록번호, 카드의 카드번호와 유효기간을 추출하는 방법에 대한 글입니다.

## CLOVA OCR

<Image
  src="/imgs/post/id-ocr/car.png"
  width="800"
  height="350"
  alt={"2년전 졸업작품, 번호판 인식 주차장 시스템"}
/>

클라우드 서비스를 사용하지않고 이미지를 직접 처리하여 글자를 추출하는것도 가능하지만,\
정확도(특히 한글)와 속도에서 크게 뒤쳐졌기에 클라우드 서비스를 이용할것 입니다.
...
> 코드 흐름은 간단하지만, 내용이 너무 길기에 알고리즘 순서도로 올렸습니다. 코드는
